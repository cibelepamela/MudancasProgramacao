...
def add_new_data(round_uuid, lap, lat, lon, vel, datetime):
    new_data = Fenrir_db(round_uuid = round_uuid, lap = lap, lat = lat, lon = lon, vel = vel, datetime = datetime)
...
def data_function():
    if request.method == 'GET':
        ...
        return add_new_data(content['round_uuid'], content['lap'], content['lat'], content['lon'], content['vel'], time.time())
...
if __name__ == '__main__':
    app.debug = True
    app.run(host='192.168.0.10', port=4996)